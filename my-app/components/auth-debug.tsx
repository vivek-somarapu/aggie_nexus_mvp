...
export default function AuthDebug() {
    ...
    const checkAuth = async () => {
      ...
      try {
        ...
        if (data.user) {
          ...
          setRls(testError ? `Failed: ${testError.code} - ${testError.message}` : 'Working')
        }
      } catch (err) {
  ...
  return (
    <div style={{ 
      ...
      <div><strong>Auth Debug</strong> (only visible in dev)</div>
      ...
      <button 
        onClick={() => console.log({ user, profile, sessionState, userState, cookies })}
