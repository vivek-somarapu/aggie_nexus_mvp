...
const fetchFounderCalendarData = unstable_cache(
  async (teamId: string) => {
  ...
  ['accel-founder-calendar'],
  { revalidate: 30, tags: ['accel-calendar'] }
);

const fetchAdminCalendarData = unstable_cache(
  ...
  ['accel-admin-calendar'],
  { revalidate: 30, tags: ['accel-calendar'] }
);

const fetchMentorCalendarData = unstable_cache(
  async (userId: string) => {
    const supabase = createAdminClient();
    const todayStr = new Date().toISOString().split('T')[0];
...
export default async function CalendarPage() {
  ...
  if (role === 'founder') {
    ...
    calendarData = await fetchFounderCalendarData(profile.team_id);
  } else if (role === 'mentor') {
    calendarData = await fetchMentorCalendarData(user.id);
  } else {
    calendarData = await fetchAdminCalendarData(role);
  }
